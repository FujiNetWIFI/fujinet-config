...
static bool bar_visible = false;

void bar_clear(bool oldRow)
{
  if (bar_visible)
  {
    screen_set_region(bar_c, bar_y + (oldRow ? bar_oldi : bar_i), 40 + SCR_X0 - bar_c, 1);
    screen_fill_region(BAR_PATTERN_OFF); // set color of 'inactive' pixels
    //screen_fill_region(MASK_KEEP_COLOR | PATTERN_REVERSE); // reverse bitmap
    bar_visible = false;
  }
}
...
void bar_update(void)
{
  if (bar_visible)
    bar_clear(true);
	screen_set_region(bar_c, bar_y + bar_i, 40 + SCR_X0 - bar_c, 1);
	screen_fill_region(BAR_PATTERN_ON); // set color of 'active' pixels
  //screen_fill_region(MASK_KEEP_COLOR | PATTERN_REVERSE); // reverse bitmap
  bar_visible = true;
}
