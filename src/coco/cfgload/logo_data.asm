; Standalone DragonDOS binary containing just the ZX0-compressed splash
; logo data (516 bytes from 4048 bytes raw), assembled directly to a
; loadable .DWL file -- kept separate from cfgload.bin so the logo doesn't
; count against that program's size budget. Fetched at runtime via
; dwload_clone("LOGO.DWL", 0) into LOGO_SCRATCH, then unpack_zx0()
; decompresses it into the screen buffer. Generated with:
; salvador -classic bitmap.bin bitmap.zx0
;
; org must be >= cfgload.bin's own --limit (0x2600, see the Makefile's
; dragon cfgload rule): this data lands here *while cfgload's own code is
; still running and using its stack* between program_end and that limit,
; so anything below the limit would corrupt the stack mid-download.
; 0x2600 itself is safe because nothing else uses it -- must match
; LOGO_SCRATCH in cfgload.c. (This is a different, higher address than
; STAGE2.DWL/CONFIG.DWL use -- see stage2.c for why CONFIG.DWL's org
; isn't bounded by cfgload.bin at all anymore.)
